...
const Header = () => {
    ...
    return (
        <nav
            ...
            } font-medium duration-500 bg-opacity-90 transition-all linear z-40 dark:text-white w-[85%] sm:w-[80%] md:w-[80%] lg:w-[55%] xl:w-[50%] max-w-6xl mx-auto  bg-white dark:bg-[#353535] drop-shadow-xs backdrop-blur-sm top-4 sticky rounded-2xl `}
        >
            <div className="flex justify-between md:space-x-10 lg:space-x-12 xl:space-x-16 md:flex items-center place-items-center md:justify-center py-3 md:px-10 px-8">
                ...
                >
                    {Links.map((link) => (
                        <li key={link.name} className="md:ml-8 text-base lg:text-lg md:my-0 my-7">
                            <Link
                                href={link.link}
